  ...
  moduleServer(id, function(input, output, session) {
    ...
    create_ae_table <- function(data, adsl_data = adsl, trt_var_adsl= 'trt01a', 
      ...
      adae1 <- bind_rows(adae_saffl, adae_total) %>%
        { if (input$ae_rel != "All") filter(., aerel == input$ae_rel) else . } %>%
        { if (input$ae_sev != "All") filter(., aesev == input$ae_sev) else . } 
      ...
      final <- bind_rows(teae_any, teae_soc1, teae_pt1) %>% 
        ...
        select('SOC_PT','Placebo','Xanomeline High Dose','Xanomeline Low Dose','Total')
    ...
    create_lb_table <- function(data, trt_var = 'trta', adsl_data = adsl, trt_var_adsl= 'trt01a',  
      ...
      final1 <- final_df %>%
        ...
        group_by(param) %>%
        complete(lbnrindn = c("Low", "Normal", "High"),
                 fill = list(Placebo = 0, Total = 0, `Xanomeline High Dose` = 0,`Xanomeline Low Dose`=0)) 
        
      
      final <- final1 %>% filter(!is.na(!!sym(par_var)) & !!sym(par_var) != "") %>%
        mutate(lbnrindn= ifelse(is.na(lbnrindn),0,lbnrindn) )%>% arrange(ord_par,lbnrindn) %>% 
        select('param','Placebo','Xanomeline High Dose','Xanomeline Low Dose','Total') 
      names(final)[1] <- " "
      return(final)
    }
    ...
    output$tbl_adlb <- renderDT({
      create_lb_table(adlbc %>% filter(param %in% input$lab_param), trt_var = "trta", adsl_data = adsl, trt_var_adsl= 'trt01a',
                      par_var = 'param', ind_var = 'lbnrind')
    })
